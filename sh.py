...
def start():
    ...
    while newGame.isOn():
        while True:
            ...
            screen.blit(FONTS["GAME-ORIGINAL-RU"].render('СУТКИ', True, (53, 54, 49)), (1070, 22))
            if len(laws) >= 10:
            ...
            for i in range(len(newGame.getPlayers())):
                        ...
                        for player_ in newGame.getPlayers():
                            if player_.getInfo().id != curr_player.getInfo().id:
                                if len(curr_player.getThoughts()) == 0:
                                    newThoughts[player_] = int((100 - player_.getInfo().stealth) - player_.getInfo().eloquence / 5 - player_.getInfo().luck / 5 - player_.getInfo().smart / 5 + curr_player.getInfo().stealth / 8 + curr_player.getInfo().luck / 6 + curr_player.getInfo().smart / 6)
                                else:
                ...
                if player_rect.collidepoint(pygame.mouse.get_pos()):
                    ...
                    if isinstance(newGame.getPlayers()[i], Liberal):
                        ...
                        #for p_ in newGame.getPlayers()[i].getThoughts().keys():
                            #screen.blit(FONTS["GAME-ORIGINAL-RU-INFO"].render('Игрок ' + str(p_.getInfo().id) + ': фашист ~' + str(newGame.getPlayers()[i].getThoughts()[p_]) + '%', True, (53, 54, 49)), (1058, 22 + 67 + 17*7 + 4))
